...
#define PREV_BLKP(bp) ((char *)bp - GET_SIZE((char *)bp - DSIZE) - DSIZE) //给定一个内存块的块指针bp，获取该内存块的前一个块的块指针
...
int mm_init(void)
{
    ...
    PUT(heap_listp + (2 * WSIZE), PACK(DSIZE,1)); //序言块脚
    PUT(heap_listp + (3 * WSIZE), PACK(0, 1)); //结尾块的头
    heap_listp += (2 * DSIZE);//让heap_listp指向第一个普通块
...
static void * coalesce(void * bp) //立即合并空闲块策略
{
    ...
    if (!prev_alloc && next_alloc) //前一个快是空闲块
    {   
...
void * mm_malloc(size_t size)
{
    ...
    if (size <= DSIZE) asize = 16;//2*DSIZE 如果请求的字节数小于双字，则分配一个四字的块（包含头和脚两个字、以及一个双字的块）
    else asize = DSIZE * ((size + 15/*DSIZE + (DSIZE - 1)*/)/DSIZE); //如果请求的字节数大于双字，则内存块大小向上舍入到最近的8的倍数
    ...
    if ((bp = extend_heap(extend_size)) == NULL) return NULL; //分配失败返回NULL
    place(bp, asize);
    return bp;    
}
...
static void * find_fit(size_t chars) //首次适配策略：返回查找到的第一个大小合适的内存块
{
    ...
    while (1)
    {
        ...
        if (is_alloc && (size == 0)) return NULL; //已分配且大小为0，说明取到结尾块了

        if (!is_alloc && (size >= chars)) return bp; //未分配且大小合适，查找命中
        else bp = NEXT_BLKP(bp); //查找未命中，bp指针指向下一个块
    }
}
...
static void place(void * bp, size_t chars)
{
    ...
    if (differ < 16) chars = size; //如果原先的空闲块大小比分配块长度大不到16字节，不切割块，直接分配这个空闲块
